...
> 如果你对本文后面的内容和逻辑并不感兴趣，只是想解决“让 Codex / Claude 在本机可控地查邮件、起草邮件、确认后发邮件”这个问题，那我先一句话说清楚：把当前页面 URL 扔给你的 Codex / Claude Code，或者直接把整页复制过去，让它按下面这套 `mail-agent` 步骤照办；这比你一边翻邮箱文档一边手搓 IMAP / SMTP，要高效得多，也少踩很多没必要的坑。
