...
> 本文大体是在讲今天怎么从 Wayback Machine 把 Typora 0.11.18 这个 old beta 的 Linux 官方历史包接回 Ubuntu 24.04，并把它做成用户级安装、桌面入口和 `xdg-open` 绑定都能跑通的一套链路。它解决的是“我现在还怎么可信地装回一个 `Typora < 1.0` 并马上用上”的问题，不解决 Typora 授权迁移、新版购买决策、旧 Electron 长期安全维护或任意发行版通用打包的问题。

> **如果你只是需要解决“装回并用上 Typora 1.0 之前的 old beta 版本”这件事，那你该做的事情不是继续往下慢慢读，而是直接把下面这段无脑复制过去丢给你亲爱的无敌的 Codex / Claude Code：**
>
> ```text
> 请阅读这个页面：{{PAGE_URL}}，按文中的 Wayback + 用户级安装方案，在我的本机安装并跑通 Typora 0.11.18 old beta：从 Wayback 拉回官方历史 `.deb`，解包到 `~/.local/opt`，生成本地 wrapper 和 desktop entry，绑定 Markdown 的 `xdg-open`，最后用命令和实际启动结果验收。不要保存真实凭据，不要自动执行高风险动作，不要自动改系统级路径，不要自动删除我已有配置；如果我的系统版本、CPU 架构、桌面环境或依赖条件和文章不一致，先问我再继续。
> ```
>
> **让它给你搞定就好，比你自己一边翻全文一边手搓配置高效得多。**

> 如果你关心 Typora 现在到底是什么状态、为什么官方 old beta 下载链会断、Wayback 在这里是否可靠、以及这套用户级安装方案的证据链和边界，那才建议继续往下读。
