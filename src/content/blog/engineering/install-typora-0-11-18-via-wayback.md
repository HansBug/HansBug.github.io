...
如果你是从很多年前就开始用 Typora 的人，大概率会对它有一种很熟悉的感觉。

不是那种“功能特别多”的熟悉，而是另一种更奇怪的熟悉：它一直很安静，打开就写，写的时候不太拧巴，不太像在和一堆按钮作斗争。也正因为这个原因，很多人是从它还在 beta 时代的时候，一路用到后来的。

问题也正是从这里开始的。

当你某一天换了机器，想把一个 `Typora < 1.0` 再装回来时，会发现事情已经不是当年那个样子了。官网当然还在，支持页也在，购买页也在，历史版本入口甚至也还留着；但等你真想把 Linux 上的 `0.11.18` 这种老 beta 包拿回来，链路就突然不那么顺了。
...
先说最基本的那句：**Typora 本身就是一个 Markdown 编辑器**，而且它官方一直强调的卖点并不是“插件生态”或者“知识库系统”，而是那种尽量把写作动作做平的体验。官网首页对它的介绍很直白，大意就是一个极简的 Markdown editor and reader，核心体验是把预览和写作尽量揉在一起，减少模式切换和干扰。（见文末参考资料 1）

这也是为什么它会有一批很稳定的老用户。你不一定天天夸它，但真要换掉的时候，往往会突然意识到，哦，原来自己已经被这种工作流养刁了。

如果你已经很久没打开过它的官网，今天的首页其实还是那个熟悉的味道：很白，很轻，很安静，几乎没有故意吵你的营销堆叠。下面这张图，就是 **2026 年 4 月 13 日** 的 Typora 官方首页首屏：
...
说到这里，很多人的第一反应大概是：那不就买一个新版完事了？

理论上当然可以。

但现实往往没那么整齐。有人就是想把老工作流原样搬回来，有人只是临时在一台机器上补一个熟悉的 Markdown 编辑器，还有人单纯是因为旧 beta 用习惯了，懒得折腾新的授权和迁移。
...
而且 Wayback 这边也不是一个抽象概念。你今天真的把原始地址喂进去，它会给你一张很直白的时间线：这个官方 `.deb` 文件在归档里被抓到过多少次，大概是从什么时候开始被保存下来的。下面这张图，就是对应归档页在 **2026 年 4 月 13 日** 的样子：
...
既然要捞 old beta，就还有一个问题：为什么偏偏是 `0.11.18`？
...
旧 Electron 包放在用户目录下时，直接起原始二进制不一定舒服。我这次实际跑通的方式，是补一个很薄的 wrapper，并且显式带上 `--no-sandbox`。
...
因为很多时候你真正想要的不是“程序装上了”，而是“以后双击 `.md` 文件、或者 `xdg-open README.md`，都别再给我跑去别的编辑器里了”。
...
对这种旧 Electron 程序，`typora --version` 不是一个总能让人舒服的测试方式。
