...
| 样本层 | session 数 | 占全样本比 | `codex / claude_code` | tool call 保留率 |
| --- | ---: | ---: | ---: | ---: |
| 全样本 | `1,664` | `100.0%` | `1,262 / 402` | `100.00%` |
| 工作流子样本 | `1,127` | `67.7%` | `998 / 129` | `99.98%` |
...
先看会话规模。表 8 与图 6 说明，这次样本分层并不是在“任意删数据”。被排掉的 `537` 条轻问答会话，占总 `session` 的 `32.3%`，但只带走了 `0.02%` 的 `tool call`；与此同时，工作流子样本依然覆盖全部 `48` 个活动日。这意味着研究三真正被抽出来的，并不是一个更小、更偏的样本，而是把大量“问两句就走”的轻问答剥掉之后，留下来的那部分真正进入执行态、回流态和裁决态的工作会话。
