...
本文的实证部分最终拆成三项互补研究。研究一看作者级整体工作面，回答“这些年我平常到底在贡献什么”；研究二看公开可复核的 `14` 天峰值窗口，回答“高强度时间窗里公开表面究竟能压到什么程度”；研究三则转向真实 `Codex / Claude Code` 使用日志，回答“这些产出背后，agent 是怎样被接进工作流的”。三项研究分别承担不同层级的问题，因此每一项都在自己的小节内单独给出 `定义 / 实验设计 / 实验结果`。

三项研究共用的边界与原则有四条：

1. 统计单位统一按“人”而不是按“单仓库”组织；仓库只是工作面的一个切片，不是本文真正的研究对象。
2. 公开细表、外链与可复核窗口，只使用 `HansBug` 这边已经公开的信息；真实 agent 日志实验则只讨论我自己真实使用过程中留下的数据源，不把来源细节与路径级敏感信息直接写进正文。
3. Git 侧统计的统一截止时间仍然是 **2026-04-14**；日志侧当前采用 **2026-04-16** 的导出批次作为样本截止点，后续若继续补样，则统一按 export batch 记录导出时间。
4. 总体研究设计这一节只保留全体实验共用的边界、比较原则与证据分层；具体到某项研究内部到底怎么取样、怎么清洗、怎么定义指标，都写回各自的小节里，而不是在总设计里一锅乱炖。

也正因为如此，下面三项研究虽然都在为同一篇文章服务，但它们并不是简单重复，而更像三种互相咬合的镜头：先看工作面，再看公开峰值，再看 agent 工作流本身。这样后面回答 `RQ` 时，才不会一会儿拿 Git 说会话，一会儿拿日志说产能，最后把坐标系搅成一锅粥。

### 研究一：作者级技术栈画像与清洗后的整体结构
...
### 研究三：Codex / Claude Code 真实使用日志

#### 定义：从真实 agent 日志观察工作流内部机制

前面两项研究说明了 Git 表面的产出结构，却还没有真正回答另一个更深一层的问题：这些产出背后，agent 到底是以怎样的密度、怎样的并行方式、怎样的人工裁决节奏被接进工作流的。也正因为如此，这里单独引入一项真实日志研究，基于 `Codex` 与 `Claude Code` 的真实使用记录，把会话级、任务级和切换级证据补齐。

这组研究的边界也得先说清楚：它主要服务于 `RQ2` 与 `RQ3`，也主要对应 Vibe / Agent 期，因为前 AI 基线期与初步 AI 期并不存在同等粒度、同等形态的 agent 历史日志。换句话说，它更适合回答“今天的工作流究竟是怎么被改写的”，而不直接承担跨三时期总量倍率的比较任务。

更重要的一点是，这一项研究从一开始就必须按“分批导出、统一 schema、下游合并”的方式设计，而不是把某一次导出误当成最终全貌。真实工作流记录本来就不会天然整齐地躺在同一份文件里，只有先把批次边界、去重规则和汇总口径钉死，后面的并行度、切换密度与人工裁决负担才有资格拿出来谈。

#### 实验设计：以统一导出 schema 组织分批日志并做汇总分析

本研究当前采用的基础工具，是 [HansBug/agent-records-exporter](https://github.com/HansBug/agent-records-exporter) 这一套统一导出器[@agent-records-exporter]。它会同时读取 `~/.codex` 与 `~/.claude`，并统一导出 `session`、`message`、`turn`、`tool_call`、`history`、`log` 与 `event` 七类记录；输出层则同时保留 `agent_records.parquet`、若干 CSV 明细表以及 `summary.json`。这一点很关键，因为后续真正做分析时，既需要 long-table 层的统一抽样，也需要按表检查细节和做人工复核。

从字段上看，这套导出器已经足够支撑一组严肃的工作流级指标。为了避免后面只剩下一串 schema 名字，先把研究三真正会用到的关键字段与用途摊成表：
...
研究三真正关心的，不是 exporter 里一共有多少字段，而是如何把它们组织成一套能回答问题的样本口径。实验设计在这里收得尽量简单。

#### 实验设计：两层样本、六组问题与统一口径

本研究先按稳定指纹对 `session` 去重，再把 `cwd / git_origin_url` 等上下文字段匿名化成稳定锚点。后文所有计数都以去重后的 `session` 为主分析单元，`turn` 与 `tool call` 只作为下钻层；强度指标统一报告中位数与 `P90`。

为避免轻问答把工作流讨论带歪，研究三把样本分成两层：`全样本` 用来回答会话规模与字段覆盖；`工作流子样本` 用来回答真正与工作方式有关的交互密度、摩擦、并行与人工裁决负担。后者只剔除同时满足 `turn_count <= 2`、`user_message_count <= 2` 且 `tool_call_count <= 3` 的 `session`。这类会话更像“问两句就走”的轻问答；但只要已经进入明显执行态，即便轮数不高，也仍然保留。

具体来说，`会话规模` 主要看全样本，因为它关心的是整个工作流在观测窗内到底铺开到了什么程度；`工作流锚定` 先看全样本覆盖，再看工作流子样本是否足以支撑后续解释；`交互密度`、`会话完成度与摩擦`、`并行与切换` 与 `人工裁决负担` 则统一转入工作流子样本，因为这些问题真正关心的是“在正经干活的会话里，工作方式是什么样子”，而不是轻问答把统计表面冲成了什么样子。

在这个前提下，后文结果只围绕六个问题展开：会话规模、交互密度、会话完成度与摩擦、工作流锚定、并行与切换，以及人工裁决负担。为了避免这些名字听起来像经验判断而不是研究对象，这里先给出本文采用的操作性定义：

- `会话规模`：指观测窗口内可辨认任务线的展开广度，核心量是去重后的 `session` 总数 $S = |\mathcal{S}|$、有活动日期数 $A = |\{d \mid n_d > 0\}|$，以及活动日均会话数 $\bar{n} = \frac{1}{A}\sum_d n_d$。它反映的是工作流到底铺开到了什么程度，而不是模型被点开了多少次。
- `交互密度`：指一条任务线推进时需要消耗多少轮往返与多少次工具调用。本文主要看每个 `session` 的时长、`turn` 数、`tool call` 数，以及总体工具密度 $D_{\text{tool/turn}} = \frac{N_{\text{tool}}}{N_{\text{turn}}}$。它反映的是 agent 只是点缀式帮忙，还是已经深度嵌入主流程。
- `会话完成度与摩擦`：前者看任务线能否顺利收口，后者看推进过程中有多少中断、异常与返工。完成度可粗略写成 $R_{\text{comp}} = \frac{N_{\text{completed}}}{N_{\text{turn}}}$；摩擦则由 `aborted/open turn`、`issue_session`、`history mismatch` 与非零退出等代理指标共同表征。它反映的是流程是否稳定、顺滑。
- `工作流锚定`：指一条会话能否被重新放回真实工程上下文里，例如落在哪个 workspace、仓库、模型档位与权限边界下。本文对每个字段都看一个覆盖率，形式上可写成 $C_f = \frac{N_f^{\text{obs}}}{N_{\text{session}}}$。它反映的是这套分析是不是建立在真实工作上下文上，而不是飘在抽象聊天层。
- `并行与切换`：前者看同一时间里有多少条任务线同时活着，后者看人需要多频繁地在不同上下文之间来回切换。并行度使用时间加权平均并发 $\bar{c} = \frac{\int c(t)\,dt}{\int \mathbf{1}_{c(t) > 0}\,dt}$；切换则按相邻会话锚点变化次数统计。它反映的是所谓“多线程工作”到底是不是实打实存在。
- `人工裁决负担`：指人在多大程度上必须持续介入、纠偏、追问和重新定边界。本文不直接声称测量心理疲劳，而是使用 `>=2` 次用户消息占比、`commentary / final answer` 比、`issue_session` 占比等代理指标，例如 $B_{2+} = \frac{N_{\text{user}\ge 2}}{N_{\text{session}}}$。它反映的是 AI 进场之后，人究竟是被解放了，还是被推向了更高密度的裁决工作。

如果要把这里最关键的一条并行定义单独写成块级公式，那么就是：

$$
\bar{c} = \frac{\int c(t)\,dt}{\int \mathbf{1}_{c(t) > 0}\,dt}
$$

它表示的不是“某个瞬间最多开了多少个会话”，而是整个活跃时间区间里，平均有多少条任务线同时在活着。

#### 实验结果：六组问题逐项展开

#### 表 8：研究三样本分层与会话规模摘要

| 样本层 | session 数 | 占全样本比 | 活动日数 | 活动日日均 `session` | 活动日 `P90` | `codex / claude_code` | tool call 保留率 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 全样本 | `1,664` | `100.0%` | `48` | `29.73` | `65.60` | `1,262 / 402` | `100.00%` |
| 工作流子样本 | `1,127` | `67.7%` | `48` | `23.48` | `48.90` | `998 / 129` | `99.98%` |
| 被排除轻问答 | `537` | `32.3%` | `-` | `-` | `-` | `264 / 273` | `0.02%` |

```mermaid
%% series-labels: 全样本|工作流子样本
%% series-colors: #8d99ae|#8fd6ff
xychart-beta
    title "图 6 活动日 session/day 分箱分布"
    x-axis ["1-10", "11-20", "21-30", "31-40", "41-60", "61+"]
    y-axis "Active Days" 0 --> 20
    bar [15, 7, 6, 6, 7, 7]
    bar [17, 8, 5, 10, 7, 1]
```

先看会话规模。这里特意把总量统计和分布图拆开：表 8 负责交代 `session` 总数、活动日数、活动日日均与 `P90`，图 6 则只看活动日 `session/day` 的分箱分布形状，避免把总量、位置统计和分布本身硬塞进同一张图里。这样一来，读者既能看到“到底有多少”，也能直接看到“这些会话到底是如何铺在每天上的”。

表 8 与图 6 合在一起说明，这次样本分层并不是在“任意删数据”。被排掉的 `537` 条轻问答会话，占总 `session` 的 `32.3%`，但只带走了 `0.02%` 的 `tool call`；与此同时，工作流子样本依然覆盖全部 `48` 个活动日，活动日日均也仍然高达 `23.48`。这意味着研究三真正被抽出来的，并不是一个更小、更偏的样本，而是把大量“问两句就走”的轻问答剥掉之后，留下来的那部分真正进入执行态、回流态和裁决态的工作会话。

#### 表 9：研究三交互密度指标（工作流子样本）

| 指标 | 结果 |
| --- | ---: |
| `session` 时长中位数 / `P90` | `32.68 / 156.81` 分钟 |
| 每个 `session` 的 `turn` 中位数 / `P90` | `1 / 8` |
| 每个 `session` 的 `tool call` 中位数 / `P90` | `99 / 417` |
| `tool call / turn` | `53.68` |
| `commentary / final answer` 比 | `11.89` |

```mermaid
%% series-labels: session 数
%% series-colors: #8fd6ff
xychart-beta
    title "图 7 工作流子样本的工具调用分布"
    x-axis ["0-9", "10-29", "30-99", "100-299", "300+"]
    y-axis "Sessions" 0 --> 400
    bar [85, 209, 272, 360, 201]
```

再看交互密度。这里已经完全转入工作流子样本。`tool call` 中位数达到 `99`，`P90` 达到 `417`；`session` 时长中位数 `32.68` 分钟，`P90` 更是达到 `156.81` 分钟。图 7 则把分布形状直接摆了出来：`30-99`、`100-299` 和 `300+` 这三段加起来占了大多数，会话明显不是停留在“问一句、答一句”的浅层交互里。换句话说，今天的高互动并不是被轻量问答样本“冲淡”或“抬高”的统计幻觉，而是工作流子样本本身就长期停留在执行、回看、追问和继续推进的中间态。

#### 表 10：研究三完成度与摩擦指标（工作流子样本）

| 指标 | 结果 |
| --- | ---: |
| `turn` 完成 / 中断 / 未闭合 / superseded | `90.6% / 7.3% / 1.8% / 0.3%` |
| `aborted_or_incomplete` session 占比 | `24.0%` |
| `issue_session` 占比 | `52.8%` |
| `history_mismatch_session` 占比 | `2.0%` |
| 已知退出码中的非零退出占比 | `5.13%` |

```mermaid
%% series-labels: 比例
%% series-colors: #f2c16d
xychart-beta
    title "图 8 工作流子样本的完成度与摩擦"
    x-axis ["完成率", "中断率", "未闭合率", "Issue", "History", "非零退出"]
    y-axis "Percent" 0 --> 100
    bar [90.6, 7.3, 1.8, 52.8, 2.0, 5.13]
```

会话完成度与摩擦则呈现出一种很典型的“双高”结构。工作流子样本里，`turn` 完成率仍有 `90.6%`，说明整条流水线不是到处失控；但 `aborted_or_incomplete` session 占比依旧达到 `24.0%`，`issue_session` 也还有 `52.8%`。也就是说，今天的 agent 工作流并不是“做不成事”，而是“做得成事，但中间充满了裁决、改口、回退和重喂上下文的节点”。图 8 把这一点看得更直白：真正低的是 `history mismatch` 与显式非零退出，高的是宽口径摩擦指标。这更像是高强度工作流里的正常扰动，而不是简单的失败。

#### 表 11：研究三关键锚定字段覆盖率

| 字段 | 全样本 | 工作流子样本 |
| --- | ---: | ---: |
| `cwd / anchor_key` | `99.2%` | `100.0%` |
| `model_provider` | `85.8%` | `100.0%` |
| `approval_mode` | `75.6%` | `88.9%` |
| `sandbox_policy` | `75.4%` | `88.6%` |
| `git_branch` | `67.7%` | `81.7%` |
| `model` | `69.4%` | `80.2%` |
| `reasoning_effort` | `60.5%` | `69.1%` |
| `git_origin_url` | `57.3%` | `69.7%` |

```mermaid
xychart-beta
    title "图 9 工作流子样本的关键锚定字段覆盖率"
    x-axis ["工作区", "提供方", "审批", "沙箱", "分支", "模型", "推理", "仓库"]
    y-axis "Coverage %" 0 --> 100
    bar [100.0, 100.0, 88.9, 88.6, 81.7, 80.2, 69.1, 69.7]
```

工作流锚定这一层，在剔除轻问答之后反而变得非常扎实。`cwd / anchor_key` 与 `model_provider` 覆盖率都达到 `100%`，`approval_mode / sandbox_policy` 也接近九成，`git_branch / model / reasoning_effort / git_origin_url` 这些更容易缺失的字段也都保持在可用区间。换句话说，研究三后面讨论的并不只是抽象聊天，而是能被重新放回真实工程上下文中的工作记录。这一点如果站不住，后面的并行、切换和人工裁决其实都会飘。

#### 表 12：研究三并行与切换指标（工作流子样本）

| 指标 | 结果 |
| --- | ---: |
| 可恢复时间区间的 `session` 占比 | `100.0%` |
| 时间加权平均并发 / 峰值并发 | `2.75 / 12` |
| distinct workspace 中位数 / `P90` | `4.0 / 10.30` |
| workspace switch 中位数 / `P90` | `7.5 / 22.30` |
| `>=3` 个 workspace 锚点的活动日占比 | `70.8%` |

```mermaid
%% series-labels: 活动日数
%% series-colors: #8fd6ff
xychart-beta
    title "图 10 工作流子样本的每日切换分布"
    x-axis ["0-2", "3-5", "6-9", "10-14", "15+"]
    y-axis "Active Days" 0 --> 14
    bar [12, 9, 6, 10, 11]
```

并行与切换这一层也直接转入工作流子样本。时间加权平均并发仍然有 `2.75`，峰值并发则达到 `12`；按日看，distinct workspace 中位数是 `4`，workspace switch 中位数是 `7.5`。更关键的是，图 10 把切换分布直接摊开之后，可以看到 `10-14` 与 `15+` 这两档活动日并不罕见。这说明今天的问题已经不是“同一天是不是做了很多事”，而是多条任务线确实在同时活着，人需要持续在不同上下文之间来回切换。

#### 表 13：研究三人工裁决负担指标（工作流子样本）

| 指标 | 结果 |
| --- | ---: |
| `>=2` 次用户消息的 session 占比 | `43.9%` |
| `>=3` 次用户消息的 session 占比 | `31.9%` |
| `commentary / final answer` 比 | `11.89` |
| `混合流水线型` session 占比 | `41.9%` |
| `混合流水线型` tool call 占比 | `76.0%` |
| `实现改写型 / 验证执行型 / 检索调研型` session 占比 | `22.4% / 14.6% / 8.7%` |

```mermaid
%% series-labels: session 数
%% series-colors: #ff9bb0
xychart-beta
    title "图 11 工作流子样本的用户指令分布"
    x-axis ["0-1", "2", "3-4", "5-7", "8+"]
    y-axis "Sessions" 0 --> 700
    bar [632, 136, 114, 93, 152]
```

最后看人工裁决负担。这里也只看工作流子样本。`43.9%` 的 `session` 至少需要两次用户消息，`31.9%` 甚至需要三次以上；图 11 则说明，真正形成厚尾的并不只是 `2` 或 `3-4` 次，而是 `8+` 这一档仍然保有相当规模。与此同时，`混合流水线型` 会话只占 `41.9%` 的 `session`，却吃掉了 `76.0%` 的工具调用。换句话说，今天人的主要工作并不是替 agent 把每一步亲手做完，而是持续开任务、盯回流、做判断、裁剪边界，以及在关键节点上拍板。

把研究三和前两项研究合在一起，`RQ2` 至少有一部分已经可以说得很硬：今天的 AI coding，绝不是 IDE 里多了一个补全框那么简单；它更像是一层新的工作组织系统。人在里面干的不再主要是把每一行代码亲手敲完，而是持续地开任务、盯回流、看执行、压边界、做裁决、切上下文。代码当然还在写，而且写得比过去更快；但“写代码”本身，早就不再足以描述整套工作的真实结构了。

## 回到开头的三个问题

前面的研究设计、样本与结果，主要是在把坐标系钉死；从这里开始，才真正回到开头那三个 `RQ`。不过也得先把话说清楚：`RQ1` 现在已经可以靠公开可复核样本给出比较硬的回答；`RQ2` 在研究三补完之后，也终于有了一条比较完整的工作流级证据链；真正还不适合抢跑的，主要只剩 `RQ3`。也正因为如此，这一节里会先把前两个问题回答得更硬一些，再把最后一个问题留给后面的杂谈与后续补实验。

### RQ1：如果把时间线、分期和可复核样本都摆上来，今天的产能变化到底有多大？

这个问题其实已经不太含糊了。只要把前面的分期、公开窗口、清洗规则和落地层指标一起摆上来，结论就很难再往“小幅提升”那边糊弄：今天的产能变化，已经不是“有点快了”，而是工作单元被重写之后，整体输出规模和并行度都发生了量级切换。

#### 表 14：公开样本的关键倍率（更贴近“干成了多少事”）
...
表 14 把公开样本里几组最关键的倍率直接压到了一张表里。只从公开可复核窗口来看，当前 Vibe / Agent 期相对于前 AI 公开窗口，在 `14 天 commit contribution` 上是 `5.43x`，在 landed commit 上是 `10.65x`，在有效变更行上更是达到了 `119.59x`；相对于初步 AI 公开窗口，几个主要指标也仍然稳定在 `2.4x` 到 `24.5x` 之间。
...
这个问题和 `RQ1` 不一样，不能只拿产能倍率直接下结论。对笔者而言，`RQ2` 的正式回答至少要建立在三层证据之上：研究一回答整体工作面如何迁移，研究二回答峰值窗口里并行推进与落地结构如何变化，研究三回答 `Codex / Claude Code` 的真实使用密度、任务并行形态与人工裁决节点到底是什么样子。现在三层证据都已经到位，这个问题其实已经能比上一版说得硬得多。
...
```mermaid
xychart-beta
    title "图 12 公开样本：平均同时活跃仓库数 / 日"
    x-axis ["前AI公开样本", "初步AI公开样本", "Vibe公开样本"]
    y-axis "Repos / day" 0 --> 3
    bar [0.79, 1.0, 2.43]
```

图 12 只看公开可复核层面的并行度变化：今天问题已经不是“一个仓写得快不快”，而是同一个人能不能同时把几条线都挂起来推进。

而研究三则把这件事的内部机制彻底钉死了。把那 `537` 条轻问答会话剔掉之后，工作流子样本仍然覆盖全部 `48` 个活动日，并保留了 `99.98%` 的 `tool call`；在这个更接近“正经干活”的样本上，`tool call` 中位数直接抬到 `99`，时间加权平均并发仍有 `2.75`，workspace switch 中位数仍有 `7.5`，而且 `43.9%` 的 `session` 至少需要两次用户消息。与此同时，`混合流水线型` 会话只占 `41.9%` 的 `session`，却吃掉了 `76.0%` 的工具调用。这几层证据合起来，其实已经足够说明：今天的 AI coding，把工作单元从“一个人盯着一个仓库亲手推进”改写成了“人站在上游调度多条任务线，让 agent 在下游持续执行和回流”的组织化工作流。

因此，`RQ2` 的回答已经可以直接写成一句话：AI coding 真正改写掉的，是工作流而不是单次补全。今天首先被改变的，不是某个函数多快能写完，而是任务如何被拆分、如何被并行、如何被追踪，以及人在何处必须亲自做判断。

### RQ3：这种变化到底有没有把人变轻松？

这个问题到现在也依然不能抢跑。即便 `RQ1` 已经说明产出规模怎么变了，`RQ2` 也已经把工作流级证据补齐了，“更轻松”本身仍然不是一个可以被单个指标简单封口的问题。它至少还牵涉到认知负荷、疲惫感、责任密度与主观恢复成本这些更偏体感、也更难直接量化的对象。因此，这一问目前仍然明确保留为 `TODO`，后面只先把可测部分拆出来，再等额外实验与后文杂谈把它补全。

#### 实验设计：要把“更轻松”拆成哪些可测对象

严格来说，“更轻松”根本不是单一指标，而至少包含三层。其一是单位时间产出是否提升，这部分主要由 `RQ1` 承担；其二是同等产出背后的人类介入密度是否下降，这需要 `RQ2` 与研究三的日志证据；其三则是上下文切换、待裁决任务数与高强度脑力节点是否反而上升，这一层才真正接近“轻松与否”的主观体感。也正因为如此，这一问不能只看 commit，也不能只看 session 数，必须把产出、并行度与认知负荷一起放进同一个框架里。

#### TODO：后续需要补的实验

- `TODO`：待 `RQ1`、`RQ2` 与研究三都基本补齐后，再在这里给出 `RQ3` 的正式回答。
