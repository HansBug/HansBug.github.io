...
第一组是 `session_key / agent_type / session_id`。这三者决定了后续所有分析能不能站稳。原因很简单：研究三不是在做一份聊天导出，而是在做一份跨 `Codex` 与 `Claude Code` 的统一工作流数据集。如果连最基本的会话主键都立不住，那么后面的 turn、tool call、history 和 log 就都只能是一堆散片，谈不上什么严肃分析。

第二组是 `cwd / git_branch / git_origin_url`。这组字段的重要性，怎么强调都不为过。因为本文真正关心的不是“今天模型说了多少话”，而是“这些会话究竟锚在什么工程上下文里”。`cwd` 把会话拉回到具体目录，`git_branch` 说明它当时落在哪条开发支线上，`git_origin_url` 则进一步把目录重新锚回远端仓库。没有这组字段，后面要谈跨 repo 切换、多任务并行、同日多仓回流，几乎都只能凭感觉；有了这组字段，很多原本只是体感的东西才第一次变成了可测对象。
...
从正式合并结果看，关键字段的可用性也足以支撑后文分析。合并样本里，`session` 级字段中，`cwd` 非空 `1651 / 1664`，`git_branch` 非空 `1127 / 1664`，`git_origin_url` 非空 `953 / 1664`，`approval_mode` 非空 `1258 / 1664`，`sandbox_policy` 非空 `1254 / 1664`，`model_provider` 非空 `1427 / 1664`，`model` 非空 `1154 / 1664`，`reasoning_effort` 非空 `1007 / 1664`，`duration_seconds` 非空 `1426 / 1664`。换句话说，repo 锚定、模型档位、审批策略、沙箱策略与时长密度这一层，已经不是“偶尔有几个样本能看”的程度，而是足以支撑系统统计。

基于这些已知字段，本文在研究三里实际按六组指标展开：
...
4. `工作流锚定`：`cwd`、`git_branch`、`git_origin_url`、`model_provider`、`model`、`reasoning_effort`、`approval_mode`、`sandbox_policy`，用于把 agent 会话重新锚回真实工程上下文，而不是只看聊天文本。
...
本次实际执行时，日志先按导出批次独立抽取，再在汇总阶段补批次元信息后做 union。正文层完全不展示来源拆分；同时，对 `cwd / git_origin_url` 只保留匿名 hash 锚点，不展示任何仓库名、目录名原文与消息正文。为避免极少数跨批次重复记录把样本虚胖，合并时再用 `agent_type + session_id + created_at / updated_at + turn_count + tool_call_count` 构造稳定指纹去重。最终只去掉了 `2` 条重复 session，说明各批次样本基本互补而不是彼此覆盖。

#### 实验结果：合并样本中的工作流密度、切换与任务形态

#### 表 8：研究三合并样本的总体规模

| 指标 | 合并结果 |
| --- | ---: |
| 观测时间范围 | `2026-02-25` ~ `2026-04-16` |
| 有活动日期 | `48` 天 |
| session 总数 | `1,664` |
| 其中 `codex / claude_code` | `1,262 / 402` |
| turn 总数 | `4,350` |
| tool call 总数 | `217,113` |
| user / assistant message 总数 | `5,265 / 51,060` |
| commentary / final answer 总数 | `43,346 / 3,737` |
| 去重后重复 session | `2` |

表 8 先把总盘子摆出来。`2026-02-25` 到 `2026-04-16` 这 `48` 个有活动日里，合并样本一共得到 `1,664` 个 session、`4,350` 个 turn 与 `217,113` 次 tool call；如果按活动日均值来算，已经是 `29.73` 个 session / day 的强度。与此同时，`assistant_message` 总量达到 `51,060`，其中 `commentary` 就有 `43,346` 条，而真正的 `final answer` 只有 `3,737` 条。这个比例本身就很说明问题：今天的 agent 使用，并不是“问一句，答一句，然后结束”，而是大量停留在中间推进、状态汇报、工具回流与持续追问之中。

#### 表 9：研究三工作流强度、完成度与切换指标

| 指标 | 合并结果 |
| --- | ---: |
| session 时长中位数 | `19.89` 分钟 |
| session 时长 P90 | `116.67` 分钟 |
| 每个 session 的 turn 中位数 | `1.00` |
| 每个 session 的 tool call 中位数 | `30.00` |
| 全样本 `tool call / turn` | `49.91` |
| `commentary / final answer` 比 | `11.60` |
| turn 完成 / 中断 / 未闭合 | `3,901 / 342 / 94` |
| 非零退出码 tool call 占比 | `5.10%` |
| `>=2` 次用户消息的 session 占比 | `30.70%` |
| 活动日均 session 数 | `29.73` |
| 可用时间区间上的平均并发 session | `2.80` |
| 可用时间区间上的峰值并发 session | `12` |
| 每个活动日的 distinct workspace 中位数 | `4.00` |
| 每个活动日的 workspace switch 中位数 | `8.50` |

表 9 其实已经把“多 agent、多线程、全程盯盘”的体感钉到数据上了。`session` 时长中位数接近 `20` 分钟，P90 超过 `116` 分钟；单个 session 的 tool call 中位数 `30`，全样本 `tool call / turn` 达到 `49.91`；`commentary / final answer` 比更是 `11.60`。这说明大部分会话并不是一次性生成后就收工，而是在一轮轮执行、看结果、再推进的链路里持续滚动。另一方面，可用时间区间上的平均并发 session 已经达到 `2.80`，峰值并发 `12`；每个活动日的匿名 workspace 中位数 `4`，workspace switch 中位数 `8.50`，而且有 `75.0%` 的活动日至少跨了 `3` 个匿名 workspace 锚点。说白了，今天真正累人的地方，已经不是“写一段代码花多久”，而是人需要长期站在多条同时推进的任务线之上，持续地做调度、裁决与回流。

这里还得补一句方法论上的冷水：exporter 里的 `issue_session` 标记非常保守，凡是 history 对齐差异、Claude agent 子会话 `session id` 偏移这类情况都会被记进去。因此，这一项更适合作为“会话不规则性上界”，不能直接拿来当工作流失败率解读。真正更稳妥的，是把它当作后续人工抽样与数据质量复核的入口，而不是 headline 指标本身。

为了避免按具体工作内容做类型学，下面这张类型表不看 repo 名称、目录原文或消息正文，只看工具结构：

- `混合流水线型`：`patch_apply >= 1` 且 `exec_command >= 3` 且 `web_search >= 1`
- `实现改写型`：`patch_apply >= 1`，但不满足上式
- `验证执行型`：`exec_command >= 5` 且 `web_search <= 2`
- `检索调研型`：`web_search >= 3` 且 `patch_apply = 0`
- `编排审阅型`：`tool_call <= 3`
- `轻量混合型`：其余情况

#### 表 10：研究三匿名会话类型分布

| 会话类型 | session 数 | session 占比 | tool call 数 | tool call 占比 |
| --- | ---: | ---: | ---: | ---: |
| 实现改写型 | `253` | `15.20%` | `35,753` | `16.50%` |
| 验证执行型 | `165` | `9.90%` | `5,036` | `2.30%` |
| 检索调研型 | `98` | `5.90%` | `8,593` | `4.00%` |
| 混合流水线型 | `472` | `28.40%` | `164,981` | `76.00%` |
| 编排审阅型 | `550` | `33.10%` | `53` | `0.00%` |
| 轻量混合型 | `126` | `7.60%` | `2,697` | `1.20%` |

```mermaid
pie showData
    title 图 6 研究三：匿名会话类型分布（按 session 占比）
    "编排审阅型" : 33.1
    "混合流水线型" : 28.4
    "实现改写型" : 15.2
    "验证执行型" : 9.9
...
如果只看 session 数，最大类不是实现改写，而是 `编排审阅型`，占到 `33.1%`；但如果看 tool call，占绝对大头的却是 `混合流水线型`，它只占 `28.4%` 的 session，却吃掉了 `76.0%` 的工具调用。这个反差非常有意思：一方面，笔者今天确实有大量时间花在编排、审阅、交接、追问和局部裁决上；另一方面，真正把算力、搜索、终端执行与代码改动一起拉满的重负载工作，则集中落在那批“混合流水线型”会话里。`实现改写型` 会话占 `15.2%`、`验证执行型` 占 `9.9%`、`检索调研型` 占 `5.9%`，它们当然都还在，但已经不再构成整套工作流的全部。也正因为如此，今天说自己“在写代码”，很多时候其实只是描述了整条流水线里最显眼的一小段，而不是全部工作。

#### 表 11：研究三主要工具结构

| 工具 | 调用数 | 占比 |
| --- | ---: | ---: |
| `exec_command` | `141,098` | `65.00%` |
| `write_stdin` | `36,184` | `16.70%` |
| `web_search` | `22,022` | `10.10%` |
| `apply_patch` | `10,186` | `4.70%` |
| `update_plan` | `2,529` | `1.20%` |
| `view_image` | `2,293` | `1.10%` |
| `Bash` | `890` | `0.40%` |
| `Read` | `817` | `0.40%` |

表 11 把工作流的“骨架工具”直接摆了出来：`exec_command` 一项就占 `65.0%`，`write_stdin` 占 `16.7%`，`web_search` 占 `10.1%`，`apply_patch` 占 `4.7%`。这基本意味着，今天的 agent 工作流主干并不是单纯的“生成代码”，而是“终端执行 -> 看结果 -> 持续追问 -> 必要时写补丁 -> 再执行”的滚动回路。换句话说，真正支撑这套工作方式的，不是某个会不会补全函数的模型，而是一整套把搜索、执行、补丁、上下文回流和人工裁决捏在一起的 agent 操作链。另一方面，`cwd` 与匿名 workspace 锚点的覆盖率都达到 `99.2%`，`git_branch` 覆盖率 `67.7%`，`git_origin_url` 覆盖率 `57.3%`，`approval_mode` 与 `sandbox_policy` 都在 `75%` 上下。这意味着前面关于多 workspace 切换、审批边界与运行约束的讨论，并不是凭空想象，而是已经有相当扎实的底层字段支撑。
...
表 12 把公开样本里几组最关键的倍率直接压到了一张表里。只从公开可复核窗口来看，当前 Vibe / Agent 期相对于前 AI 公开窗口，在 `14 天 commit contribution` 上是 `5.43x`，在 landed commit 上是 `10.65x`，在有效变更行上更是达到了 `119.59x`；相对于初步 AI 公开窗口，几个主要指标也仍然稳定在 `2.4x` 到 `24.5x` 之间。
...
```mermaid
xychart-beta
    title "图 7 公开样本：平均同时活跃仓库数 / 日"
    x-axis ["前AI公开样本", "初步AI公开样本", "Vibe公开样本"]
    y-axis "Repos / day" 0 --> 3
    bar [0.79, 1.0, 2.43]
```

图 7 只看公开可复核层面的并行度变化：今天问题已经不是“一个仓写得快不快”，而是同一个人能不能同时把几条线都挂起来推进。

而研究三则把这件事的内部机制彻底钉死了。合并样本显示，`48` 个活动日里一共出现了 `1,664` 个 session，活动日均 `29.73` 个；可用时间区间上的平均并发 session 达到 `2.80`，峰值并发 `12`；匿名 workspace 中位数 `4`，workspace switch 中位数 `8.50`，并且有 `75.0%` 的活动日至少跨了 `3` 个匿名 workspace 锚点。与此同时，`编排审阅型` 会话占到 `33.1%`，但工具调用几乎可以忽略；真正吃掉 `76.0%` 工具调用的，是只占 `28.4%` 的 `混合流水线型` 会话；工具主干则明确落在 `exec_command / write_stdin / web_search / apply_patch` 这一条终端驱动链上。这几层证据合起来，其实已经足够说明：今天的 AI coding，把工作单元从“一个人盯着一个仓库亲手推进”改写成了“人站在上游调度多条任务线，让 agent 在下游持续执行和回流”的组织化工作流。
